...
describe('LOT 4-M — moteur multi-module (registre canonique)', () => {
  ...
  it('module RÉEL Smart Money (LOT 4-W) : 12e module (5 compétences), checkpoint PROPRE et indépendant', () => {
    expect(isCheckpoint(SMC_CHECKPOINT_ID)).toBe(true);
    expect(
      new Set([CHECKPOINT_ID, CANDLE_CHECKPOINT_ID, STRUCTURE_CHECKPOINT_ID, SR_CHECKPOINT_ID, ANATOMY_CHECKPOINT_ID, PATTERNS_CHECKPOINT_ID, INDICATORS_CHECKPOINT_ID, VOLUME_CHECKPOINT_ID, PRICEACTION_CHECKPOINT_ID, RISK_CHECKPOINT_ID, PSYCHOLOGY_CHECKPOINT_ID, SMC_CHECKPOINT_ID]).size,
    ).toBe(12);
    expect(skillById(SMC_CHECKPOINT_ID)).toEqual({ id: SMC_CHECKPOINT_ID, name: SMC_CHECKPOINT_TITLE });
    const cp = checkpointExercises(SMC_CHECKPOINT_ID, 0, 2);
    expect(cp.length).toBeGreaterThan(0);
    expect(cp.every((e) => e.skillId.startsWith('skill.smc.'))).toBe(true);
  });

  it('module RÉEL Wyckoff (LOT 4-X) : 13e module (2 compétences), checkpoint PROPRE et indépendant', () => {
    expect(isCheckpoint(WYCKOFF_CHECKPOINT_ID)).toBe(true);
    expect(
      new Set([CHECKPOINT_ID, CANDLE_CHECKPOINT_ID, STRUCTURE_CHECKPOINT_ID, SR_CHECKPOINT_ID, ANATOMY_CHECKPOINT_ID, PATTERNS_CHECKPOINT_ID, INDICATORS_CHECKPOINT_ID, VOLUME_CHECKPOINT_ID, PRICEACTION_CHECKPOINT_ID, RISK_CHECKPOINT_ID, PSYCHOLOGY_CHECKPOINT_ID, SMC_CHECKPOINT_ID, WYCKOFF_CHECKPOINT_ID]).size,
    ).toBe(13);
    expect(skillById(WYCKOFF_CHECKPOINT_ID)).toEqual({ id: WYCKOFF_CHECKPOINT_ID, name: WYCKOFF_CHECKPOINT_TITLE });
  ...
  it('module RÉEL Faux signaux (LOT 4-Z) : 15e et DERNIER module (2 compétences), checkpoint PROPRE et indépendant', () => {
    expect(isCheckpoint(FALSESIGNALS_CHECKPOINT_ID)).toBe(true);
    expect(
      new Set([CHECKPOINT_ID, CANDLE_CHECKPOINT_ID, STRUCTURE_CHECKPOINT_ID, SR_CHECKPOINT_ID, ANATOMY_CHECKPOINT_ID, PATTERNS_CHECKPOINT_ID, INDICATORS_CHECKPOINT_ID, VOLUME_CHECKPOINT_ID, PRICEACTION_CHECKPOINT_ID, RISK_CHECKPOINT_ID, PSYCHOLOGY_CHECKPOINT_ID, SMC_CHECKPOINT_ID, WYCKOFF_CHECKPOINT_ID, OPTIONS_CHECKPOINT_ID, FALSESIGNALS_CHECKPOINT_ID]).size,
    ).toBe(15);
    expect(skillById(FALSESIGNALS_CHECKPOINT_ID)).toEqual({ id: FALSESIGNALS_CHECKPOINT_ID, name: FALSESIGNALS_CHECKPOINT_TITLE });
    const cp = checkpointExercises(FALSESIGNALS_CHECKPOINT_ID, 0, 2);
    expect(cp.length).toBeGreaterThan(0);
    expect(cp.every((e) => e.skillId.startsWith('skill.falsesignals.'))).toBe(true);
  });
});
