...
// Mondes 1-13 avancés (…+ Wyckoff validé) → le monde 14 (Options et volatilité) s'ouvre.
const W13_DONE = seed({
  completedSkills: [
    ...ALL_SKILLS, CHECKPOINT_ID, ...ANATOMY_DONE_IDS,
    ...CANDLE_SKILLS.map((s) => s.id), CANDLE_CHECKPOINT_ID,
...
describe('Fiche Monde de production — canon, vérité pédagogique, a11y (LOT 4-L)', () => {
  ...
  it('guidé Options (monde 14, LOT 4-Y) : ses 2 compétences + checkpoint PROPRE surfacent, prochaine étape correcte', async () => {
    // Mondes 1-13 validés → le monde 14 (Options et volatilité) est « en cours ».
    await persist(W13_DONE);
    const r = await mount('world.options');
    expect(hasText(r.root, 'Options et volatilité')).toBe(true);
    expect(hasText(r.root, 'En cours')).toBe(true);
    for (const s of OPTIONS_SKILLS) {
      const node = pressables(r.root).find((n) => String(n.props.accessibilityLabel ?? '').startsWith(`${s.name} —`));
      expect(node).toBeDefined();
    }
    expect(hasTextIncluding(r.root, OPTIONS_CHECKPOINT_TITLE)).toBe(true);
    const cta = ctaWithHint(r.root, 'Commencer la leçon');
    expect(cta).toBeDefined();
    act(() => (cta!.props.onPress as () => void)());
    expect(pushes()).toContain(`/session/${OPTIONS_SKILLS[0].id}`);
    await act(async () => r.unmount());
  });
